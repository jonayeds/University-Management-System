...
const getAssignedFaculties = catchAsync(async (req, res) => {
  const { courseId } = req.params;
  const result = await CourseServices.getAssignedFacultiesIntoCourses(
    courseId
  );
  response(res, {
    statusCode: 200,
    success: true,
    message: 'Successfully fetched assigned faculty',
    data: result,
  });
});
