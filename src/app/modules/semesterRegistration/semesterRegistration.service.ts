...
const createSemesterRegistrationIntoDB = async (
  ...
  if (isThereAnyUpCommingOnOnGoing) {
    throw new AppError(
      500,
      `There is already a ${isThereAnyUpCommingOnOnGoing.status} Semester`,
    );
  }
...
const updateSemesterRegistration = async (
  ...
  if (
    ...
    throw new AppError(
      400,
      `Cannot set status from ${isSemesterRegistrationExist.status} to ${payload.status}`,
    );
  }
...
const deleteSemesterRegistration = async (id: string) => {
  ...
  if (registeredSemesterStatus?.status !== 'UPCOMING') {
    throw new AppError(
      400,
      `Cannot delete an ${registeredSemesterStatus?.status} Semester`,
    );
  }
